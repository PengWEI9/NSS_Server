<template>
  <div class="app-container">
    <el-form ref="form" :model="form" :rules="rules" label-width="160px" :label-position="getClient()?'top':'left'" style="max-width:500px">
      ...
      <el-form-item label="Relay Start" prop="relaystart">
        <el-input v-model="form.relaystart" placeholder="Relay start" />
      </el-form-item>
      <el-form-item>
    ...
    this.$mqttClient.on('message', (topic, message) => {
      if (topic === this.currentSn + 'state') {
        ...
        try {
          ...
          if (info.relaystart !== 'undefined') {
            this.form.relaystart = info.relaystart
          }
        } catch (e) {
          //
        }
  ...
  methods: {
    updateDevice(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.$mqttClient.publish(this.currentSn + 'ctr', 'relaystart=' + this.form.relaystart)
          this.$message({
            type: 'success',
